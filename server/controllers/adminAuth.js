const Admin = require('../schema/adminAuth')
...
const register = async (req, res) => {
  const { email, password } = req.body;
    ...
    try {
      let sameEmail = await Admin.findOne({ 'email': email });

      if (sameEmail) {
        return res.status(400).json({ msg: "This email id is already in use" });
      }

      const user = await Admin.create({ ...tempUser });
...
// User Log in
  ...
  const updatePassword = async (req, res) => {
    try {
      ...
      const user = await Admin.findByIdAndUpdate(userId, { password: hashedPassword }, { new: true });
